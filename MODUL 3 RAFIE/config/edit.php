...
if (isset($_POST['simpan'])) {
    ...
    if ($namamobil && $namapemilik && $merek && $tanggalbeli && $deskripsi && $fotomobil && $status) {
        if($op = 'edit'){ //untuk edit
            $sql1 = "update showroom_rafie_table set nama_mobil = '$namamobil', pemilik_mobil = '$namapemilik', merek_mobil = '$merek', deskripsi = '$deskripsi', foto_mobil = '$fotomobil', status_pembayaran = '$status' where id_mobil = '$id'";
            $q1 = mysqli_query($koneksi, $sql1);
            if($q1){
